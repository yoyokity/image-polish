...
enum class StepKind { AA, Denoise, Resize, Dehalo, Sharpen, Deband, Grain, OnnxSr };
...
struct Step {
    ...
    std::string model;    // ONNX model path (OnnxSr)
    ...
    static Step onnxSr(std::string model) { Step s{StepKind::OnnxSr}; s.model = std::move(model); return s; }
};
